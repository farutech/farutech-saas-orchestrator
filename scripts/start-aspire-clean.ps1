...
if ($containers) {
    podman stop $containers 2>$null | Out-Null
    podman rm $containers 2>$null | Out-Null
    Write-Host "      OK Contenedores limpiados" -ForegroundColor Green
}
else {
    Write-Host "      OK No hay contenedores que limpiar" -ForegroundColor Green
}
Write-Host ""
...
Write-Host ""
Write-Host "      Ejecutando dotnet run en nueva ventana..." -ForegroundColor Gray
Write-Host "      Esto puede tardar 30-40 segundos..." -ForegroundColor Gray
Write-Host ""

# Iniciar Aspire en nueva ventana
Set-Location "D:\farutech_2025"
$aspireCmd = "cd D:\farutech_2025; dotnet run --project src\03.Platform\Farutech.AppHost"
Start-Process powershell -ArgumentList "-NoExit","-Command",$aspireCmd

Write-Host "      OK Aspire iniciando en nueva ventana..." -ForegroundColor Green
Write-Host ""
...
Write-Host "   VERIFICACION DE SERVICIOS" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
Write-Host ""
...
if ($containerCount -gt 0) {
    podman ps --format "  OK {{.Names}}: {{.Status}}"
    Write-Host "  Total: $containerCount contenedores" -ForegroundColor Green
}
else {
    Write-Host "  Aun iniciando... (esperar 10-15 segundos mas)" -ForegroundColor Yellow
}
Write-Host ""
...
if ($appHostProcess) {
    Write-Host "  OK Farutech.AppHost" -ForegroundColor Green
}
else {
...
if ($dcpctrlProcess) {
    Write-Host "  OK dcpctrl - Aspire Control" -ForegroundColor Green
}
else {
...
if ($orchestratorProcess) {
    Write-Host "  OK Orchestrator API" -ForegroundColor Green
}

if ($nodeProcesses) {
    $nodeCount = ($nodeProcesses | Measure-Object).Count
    Write-Host "  OK Frontend (Node.js: $nodeCount procesos)" -ForegroundColor Green
}
...
Write-Host "     Ver puerto en Dashboard (dinamico)" -ForegroundColor Yellow
Write-Host ""
Write-Host "========================================" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "NOTA: El warning ASPIRE004 es informativo y no afecta" -ForegroundColor Yellow
Write-Host ""
